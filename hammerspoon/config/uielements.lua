...
hs.hotkey.bind({ "cmd", "alt", "ctrl" }, "E", function()
	...
	local function testElementSearchWithFilter()
		...
		local elementCriteria = EnumTableValues(hs.axuielement.roles):filter(function(e)
			return string.find(e, "Menu")
		end):totable()
		--
...
function ElementSpecifierFor(elem)
	...
	elseif role == "AXList" then
		...
		if subrole == "AXSectionList" then
			prints(
				"WARNING: AXSubrole is AXSectionList, using 'list' but might need 'section' instead? double check and then update this warning in code and change if needed"
			)
		end
	...
	prints(
		'SUGGESTION: using roleDescription "'
			.. roleDescription
			.. '" as class (error prone in some cases), add an explicit mapping for AXRole: '
			.. role
	)
...
local function getIdentifier(toElement)
	...
	local appleScriptReservedWords = {
		"length",
		"if",
		"then",
		"else",
	...
	if EnumTableValues(appleScriptReservedWords):any(function(word)
...
function CompactUserData(userdata)
	if userdata.__name == "hs.axuielement.axtextmarker" or userdata.__name == "hs.axuielement.axtextmarkerrange" then
...
function GetDumpAXAttributes(element, skips)
	...
	for _, attrName in ipairs(sortedAttrs) do
		...
		if displayValue == "nil" or displayValue == '""' then
			table.insert(
				result,
				"\t<span class='not-set-attribute'>" .. displayName .. " = " .. displayValue .. "</span><br>"
			)
		else
			local displayNameClass = string.lower(attrName) -- used for styling important attributes (i.e. AXTitle)
			table.insert(
				result,
				"\t<span class='" .. displayNameClass .. "'>" .. displayName .. " = " .. displayValue .. "</span><br>"
			)
		end
	end
	return table.concat(result)
...
function GetElementTableRow(elem, indent)
	...
	return "<tr class='"
		...
		.. details
		.. "</td><td>"
		.. htmlCodeAppleScript(ElementSpecifierFor(elem))
		.. "</td></tr>"
end
