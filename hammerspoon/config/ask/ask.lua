...
local function selectTextToReplaceIt()
	...
	hs.eventtap.keyStroke(Config.keys.HYPER, "a", 0) -- 0ms delay b/c by the time we get any response will be way long enoughk
...
function M.grammerCheck()
	local app = hs.application.frontmostApplication() -- < 0.5ms

	selection.getSelectedTextThen(function(selectedText, focusedElem)
		doTask(selectedText, "grammerChecker", focusedElem)
	end)
end
		...
		adjustBoxElement(focusedElem, app, function(element)
			...
			askAbout(selectedText, app, focusedElem, function(textChunk)
				...
				if element then
					...
					box = hs.canvas.new({ x = x, y = y, w = tooltipWidth, h = tooltipHeight })
					...
					if #boxBindings == 0 then
						...
						table.insert(
							boxBindings,
							hs.hotkey.bind({ "cmd", "alt", "ctrl" }, hs.keycodes.map["return"], acceptBox)
						)
...
function doTask(userText, name, focusedElem, appendChunk)
	log.i("name " .. name)
	if userText == "" then
		hs.alert.show("No user text found...")
		return
	...
	local params = prompts.getSpecificParams(name, focusedElem)
	log.i(params)
	if params == nil then
		-- PRN automatic params? build system prompt off of app:name() and name of input box and then formulate a generic prompt?
		--   and treat getAppSpecificPromptAndParameters as an override?
		print("Error: unknown app - no prompt available")
		hs.alert.show("Error: unknown app - no prompt available")
		return
	end
