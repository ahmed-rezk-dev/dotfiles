...
`)),n.filter(o=>o.trim().length>0).length>0)return n}catch(n){console.error(`Couldn't get images from clipboard: ${n}`),a=!0}let s=t;try{s=(await(0,c.getFrontmostApplication)()).name}catch(n){console.error(`Couldn't get frontmost application: ${n}`)}try{(t=="Path Finder"||s=="Path Finder")&&(r=await me())}catch(n){console.error(`Couldn't get images from Path Finder: ${n}`),a=!0}try{(t=="NeoFinder"||s=="NeoFinder")&&(r=await he())}catch(n){console.error(`Couldn't get images from NeoFinder: ${n}`),a=!0}try{(t=="HoudahSpot"||s=="HoudahSpot")&&(r=await pe())}catch(n){console.error(`Couldn't get images from HoudahSpot: ${n}`),a=!0}try{(t=="QSpace"||s=="QSpace Pro"||s=="QSpace")&&(r=await ge())}catch(n){console.error(`Couldn't get images from ${s}: ${n}`),a=!0}try{(t=="ForkLift"||s=="ForkLift")&&(r=await be())}catch(n){console.error(`Couldn't get images from ForkLift: ${n}`),a=!0}if(r.length>0)return r.filter((n,o)=>r.indexOf(n)===o);let i=await de();return s=="Finder"||t=="Finder"||a?r=i:i.forEach(n=>{n.split("/").at(-2)=="Desktop"&&!r.includes(n)&&r.push(n)}),r.filter((n,o)=>r.indexOf(n)===o)},Z=async r=>{let e="Finder";try{e=(await(0,c.getFrontmostApplication)()).name}catch(a){console.error(`Couldn't get frontmost application: : ${a}`)}let t=(0,c.getPreferenceValues)();t.imageResultHandling=="copyToClipboard"?(await fe(r),ye(r)):t.imageResultHandling=="openInPreview"?(await Je(r),ye(r)):t.inputMethod=="NeoFinder"||e=="NeoFinder"?await(0,c.showInFinder)(r[0]):(t.inputMethod=="HoudahSpot"||e=="HoudahSpot")&&await(0,c.showInFinder)(r[0])},Ge=async()=>(g.cpus()[0].model.includes("Apple")?"arm":"x86")=="arm"?((0,k.execSync)(`chmod +x "${c.environment.assetsPath}/webp/arm/dwebp"`),(0,k.execSync)(`chmod +x "${c.environment.assetsPath}/webp/arm/cwebp"`),p.existsSync(`${c.environment.assetsPath}/webp/x86/dwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/x86/dwebp`),p.existsSync(`${c.environment.assetsPath}/webp/x86/cwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/x86/cwebp`),[`${c.environment.assetsPath}/webp/arm/dwebp`,`${c.environment.assetsPath}/webp/arm/cwebp`]):((0,k.execSync)(`chmod +x "${c.environment.assetsPath}/webp/x86/dwebp"`),(0,k.execSync)(`chmod +x "${c.environment.assetsPath}/webp/x86/cwebp"`),p.existsSync(`${c.environment.assetsPath}/webp/arm/dwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/arm/dwebp`),p.existsSync(`${c.environment.assetsPath}/webp/arm/cwebp`)&&await p.promises.rm(`${c.environment.assetsPath}/webp/arm/cwebp`),[`${c.environment.assetsPath}/webp/x86/dwebp`,`${c.environment.assetsPath}/webp/x86/cwebp`]),Se=async(r,e)=>{var o=[];try{let t=(0,c.getPreferenceValues)();let a=O(o,await X("tmp","png"),!0);let s=(await C([e]))[0];let[i,n]=await Ge();(0,k.execSync)(`"${i}" "${e}" -o "${a.path}" && ${r} "${a.path}" && "${n}" ${t.useLosslessConversion?"-lossless":""} "${a.path}" -o "${s}"`);return s}catch(l){var u=l,b=!0}finally{var f=_(o,u,b);f&&await f}},xe=async(r,e)=>{var o=[];try{let t=(0,c.getPreferenceValues)();let a=O(o,await X("tmp","png"),!0);let s=(await C([e]))[0];let{encoderPath:i,decoderPath:n}=await J();(0,k.execSync)(`"${n}" "${e}" "${a.path}" && ${r} "${a.path}" && "${i}" ${t.useLosslessConversion?"-s 0 --min 0 --max 0 --minalpha 0 --maxalpha 0 --qcolor 100 --qalpha 100":""}  "${a.path}" "${s}"`);return s}catch(l){var u=l,b=!0}finally{var f=_(o,u,b);f&&await f}},ve=async(r,e)=>{var s=[];try{let t=O(s,await X("tmp","bmp"),!0);let a=(await C([e]))[0];await Ke("BMP",e,t.path);(0,k.execSync)(`chmod +x "${c.environment.assetsPath}/potrace/potrace"`);(0,k.execSync)(`${r} "${t.path}" && "${c.environment.assetsPath}/potrace/potrace" -s --tight -o "${a}" "${t.path}"`);return a}catch(i){var n=i,o=!0}finally{var l=_(s,n,o);l&&await l}},Ke=async(r,e,t)=>$(`use framework "Foundation"
  ...
  flippedPdfData's writeToFile:"${a}" atomically:false`),a};var Je=async r=>{let e=Array.isArray(r)?r:[r],t=e.some(a=>d.default.extname(a)==".svg");await $(`use framework "Foundation"
    use scripting additions
  ...
  end tell`)},C=async(r,e=!1,t=void 0)=>{let a=(0,c.getPreferenceValues)(),s=await Ze(r[0]);return r.map(i=>{let n=i;if(a.imageResultHandling=="saveToDownloads"?n=d.default.join(g.homedir(),"Downloads",d.default.basename(n)):a.imageResultHandling=="saveToDesktop"?n=d.default.join(g.homedir(),"Desktop",d.default.basename(n)):(a.imageResultHandling=="saveInContainingFolder"||a.imageResultHandling=="replaceOriginal")&&(a.inputMethod=="Clipboard"||e)?n=d.default.join(s,d.default.basename(n)):(a.imageResultHandling=="copyToClipboard"||a.imageResultHandling=="openInPreview")&&(n=d.default.join(g.tmpdir(),d.default.basename(n))),n=t?n.replace(d.default.extname(n),`.${t}`):n,a.imageResultHandling!="replaceOriginal"&&g.tmpdir()!=d.default.dirname(n)){let o=2;for(;p.existsSync(n);)n=d.default.join(d.default.dirname(n),d.default.basename(n,d.default.extname(n))+`-${o}${d.default.extname(n)}`),o++}return n})},y=async(r,e,t,a)=>{console.error(e),t?(t.title=r,t.message=a??e.message,t.style=c.Toast.Style.Failure,t.primaryAction={title:"Copy Error",onAction:async()=>{await c.Clipboard.copy(e.message)}}):t=await(0,c.showToast)({title:r,message:a??e.message,style:c.Toast.Style.Failure,primaryAction:{title:"Copy Error",onAction:async()=>{await c.Clipboard.copy(e.message)}}})};var Ie=r=>{let e=g.homedir();if(r.startsWith("~"))return r.replace(/^~(?=$|\/|\\)/,e);let t=/(\/Users\/.*?)\/.*/,a=r.match(t);return a?r.replace(a[1],e):r};async function Q(r,e){let t=r.map(n=>Ie(n)),a='"'+t.join('" "')+'"',s=await C(t),i=e==0?"horizontal":"vertical";if(a.toLowerCase().includes("webp")||a.toLowerCase().includes("svg")||a.toLowerCase().includes("pdf")||a.toLowerCase().includes("avif")){let n=[];for(let o of t)if(o.toLowerCase().endsWith("webp"))n.push(await Se(`sips --flip ${i}`,o));else if(o.toLowerCase().endsWith("svg"))n.push(await ve(`sips --flip ${i}`,o));else if(o.toLowerCase().endsWith("pdf"))n.push(await Pe(o,e));else if(o.toLowerCase().endsWith("avif"))n.push(await xe(`sips --flip ${i}`,o));else{let l=s[t.indexOf(o)];n.push(l),(0,W.execSync)(`sips --flip ${i} -o "${l}" "${o}"`)}return await Z(n),n}else{let n=s.length==1?s[0]:D.default.join(D.default.dirname(s[0]),"flipped");s.length>1&&(0,W.execSync)(`mkdir -p "${n}"`),(0,W.execSync)(`sips --flip ${i} -o "${n}" ${a}`),s.length>1&&(await Promise.all(t.map((o,l)=>q.default.promises.rename(D.default.join(n,D.default.basename(o)),s[l]))),await q.default.promises.rm(n,{recursive:!0,force:!0})),await Z(s)}return s}async function qe({direction:r,imagePaths:e}){let t=e?.length?e:await ke(),a=await Q(t,r=="vertical"?1:0);return await we(),a}
