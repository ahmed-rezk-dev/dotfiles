...
-- Better way to navigate buffers
keymap("n", "<S-l>", ":BufferLineCycleNext<CR>", silent)
keymap("n", "<S-h>", ":BufferLineCyclePrev<CR>", silent)
keymap("n", "<S-x>", ":lua require('mini.bufremove').delete(0, false)<CR>", silent)
...
keymap("n", "<leader>cl", "<cmd>lua vim.diagnostic.open_float({ border = 'rounded', max_width = 500 })<CR>", silent)
keymap("n", "gl", "<cmd>lua vim.diagnostic.open_float({ border = 'rounded', max_width = 500 })<CR>", silent)
keymap("n", "<Tab>", "<cmd>lua vim.lsp.buf.signature_help()<CR>", silent)
...
keymap("n", "<leader>gL",
  function()
    ...
    -- vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, { "!git log -n 1 -L ' . line('.') . ',+1:' . expand('%:p')", 'pass bufnr 2' })
